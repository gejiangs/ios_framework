...
-(void)getADInfoWithSuccess:(void (^)(BOOL, AdModel *))success failure:(void (^)(NSError *))failure
{
    [self.requestOperator requestGetWithURL:@"http://m2.qiushibaike.com/ad?AdID=14260582945040F4E3A48F"
                                     params:nil
                                    success:^(BOOL succ, NSString *msg, id responseObject) {
                                        
                                        AdModel *adModel = [AdModel objectWithKeyValues:responseObject error:nil];
                                        success(YES, adModel);
...
-(void)getADInfoWithSuccesss:(void (^)(BOOL, AdModel *))success failure:(void (^)(NSError *))failure
{
    [self.requestOperator requestGetWithURL:@"http://m2.qiushibaike.com/ad?AdID=14260582945040F4E3A48F"
                                     params:nil
                                    success:^(BOOL succ, NSString *msg, id responseObject) {
